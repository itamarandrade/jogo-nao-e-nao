...
function createThemeDecorations(themeName) {
    ...
    switch(themeName) {
        case 'guns':
            createGunsDecorations(container);
            break;
        case 'simples':
            createSimplesDecorations(container);
            break;
        case 'natureza':
...
function createGunsDecorations(container) {
    const emojis = ['🎸', '🤘', '🌹', '🔥', '⚡', '💀', '🎵'];
    for (let i = 0; i < 10; i++) {
        ...
        item.style.top = Math.random() * 90 + 5 + '%';
...
function createSimplesDecorations(container) {
    const emojis = ['✊', '💜', '🧡', '✨', '💪'];
    for (let i = 0; i < 5; i++) {
        const item = document.createElement('div');
        item.className = 'mascara';
...
function applyConfig(config) {
    ...
    // Aplicar logo (usa logo do tema se não tiver logo customizado)
    const logoSrc = config.logo || theme.logo || 'assets/logo-png.png';
...
function generatePuzzle() {
    ...
    const puzzleImage = gameConfig.puzzleImage || (THEMES[gameConfig.theme] || {}).logo || 'assets/logo-png.png';
...
function initDashboard() {
    ...
    if (tempConfig.puzzleImage) {
        ...
        const themeLogo = (THEMES[tempConfig.theme] || {}).logo || 'assets/logo-png.png';
        document.getElementById('puzzle-preview').src = themeLogo;
    }
